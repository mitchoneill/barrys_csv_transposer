...
# Default dictionaries are great when you don't know if
# a key exists yet or not, you can just assume the key
# does exist and the (list) part means we can also
...
# key of our results list, and the values and ever growing list of DATES
    ...
    for row in reader:
        ...
        # Now when we read a data row, we want to get the value in the
        # column of our ID (the row[id_idx]) and make the our key,
        # then add the date to 
        else:
